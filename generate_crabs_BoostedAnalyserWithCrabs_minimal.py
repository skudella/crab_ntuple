...
crab_cfg_path='ZPrime_MC_minimal'
...
#        requestname="'"+(dataset.split('/')[1])+"'"
    if dataset!='' and name!='':
        ...
        crab_template=open('crab_cfg_BoostedAnalyserWithCrabs_minimal.py','r')
        for line in crab_template:
            if 'config.Data.inputDataset' in line:
                ...
                crabout.write('config.JobType.outputFiles = ["'+name+'_nominal_Tree.root", "'+name+'_nominal_Cutflow.txt"]\n')
            elif 'config.JobType.pyCfgParams' in line:
                crabout.write("config.JobType.pyCfgParams = ['outName="+name+"','weight="+weight+"','isData="+isData+"','isBoostedMiniAOD=False','systematicVariations=nominal','LeptonChannel=veto','maxEvents=9999999999','globalTag="+globalTag+"','generatorName="+generator+"']\n")
            elif 'config.Data.outputDatasetTag' in line:
                crabout.write("config.Data.outputDatasetTag = 'MC_minimal_"+name+"'\n")
            else:
                crabout.write(line)
        crab_template.close()
        crabout.close()
